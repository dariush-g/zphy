...
fn detect_collisions(mut query: Query<(Entity, &mut RigidbodyComponent, &mut Transform)>) {
    let mut items: Vec<_> = query.iter_mut().collect();

    for i in 0..items.len() {
        for j in (i + 1)..items.len() {
            // Split mutable borrow safely
            let (left, right) = items.split_at_mut(j);
            let (_, ref mut body_a, ref mut transform_a) = left[i];
            let (_, ref mut body_b, ref mut transform_b) = right[0];

            if let Some(collision_data) = get_collision_info(&body_a.collider, &body_b.collider) {
...
fn get_collision_info(a: &Collider, b: &Collider) -> Option<ContactInfo> {
